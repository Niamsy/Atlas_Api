...
router.post('/', (req, res) => {
  ...
  con.query("SELECT users.email, plant_requests.* FROM plant_requests INNER JOIN users ON users.id = plant_requests.fk_id_user WHERE plant_requests.id = " + con.escape(id_request)).then(request => {
    if (request[0].length === 0) {
      return res.status(403).json({message: "No request with the given request_id exists"});
    } else if (request[0][0].status === '1') {
      const email = request[0][0]['email'];

      const new_status = ((status === true) ? (2) : (3));
      con.query("UPDATE plant_requests SET status = " + new_status + " WHERE id = " + id_request).then(() => {
        ...
        if (status === true) {
          ...
          con.query(str_query).then(() => {
            ...
            return res.status(500).json({message: "Atlas API Encountered a issue"});
          });
