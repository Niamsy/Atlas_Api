...
router.post('/', (req, res) => {
  ...
  con
    .query(
      'SELECT users.email, plant_requests.* FROM plant_requests INNER JOIN users ON users.id = plant_requests.fk_id_user WHERE plant_requests.id = ' +
        con.escape(id_request)
    )
    .then(request => {
